      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex gap-2">
          <a :href="route('billing.cap-management.pdf', management.id)"
            class="px-3 py-1.5 text-xs bg-rose-500 text-white rounded-md hover:bg-rose-600">管理結果票PDF</a>
          <Link :href="route('billing.cap-management.index', { month: management.year_month })" class="px-3 py-1.5 text-xs border border-gray-300 rounded-md hover:bg-gray-50">戻る</Link>
        </div>
      </div>
    </template>
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-5">
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
            ...
            <div>
              ...
              <span class="inline-block text-xs bg-gray-100 px-2 py-0.5 rounded-md">{{ labels.status[management.status] }}</span>
            </div>
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-5">
          ...
          <div class="flex flex-wrap gap-2">
            <button v-if="!management.actual_confirmed_at" @click="confirmActual"
              class="px-3 py-1.5 text-xs bg-blue-500 text-white rounded-md hover:bg-blue-600">実績を確定</button>
            <button v-if="management.status === 'created'" @click="transition('send', '送付済')"
              class="px-3 py-1.5 text-xs bg-amber-500 text-white rounded-md hover:bg-amber-600">関連事業所へ送付</button>
            <button v-if="management.status === 'sent'" @click="transition('receive', '受領済')"
              class="px-3 py-1.5 text-xs bg-primary-600 text-white rounded-md hover:bg-primary-700">受領済にする</button>
            <button v-if="management.status === 'received'" @click="transition('confirm', '確定済')"
              class="px-3 py-1.5 text-xs bg-green-500 text-white rounded-md hover:bg-green-600">確定</button>
            <button v-if="['sent','received','confirmed'].includes(management.status)" @click="transition('revert', '前段階へ')"
              class="px-3 py-1.5 text-xs border border-gray-300 text-gray-600 rounded-md hover:bg-gray-50">前の状態に戻す</button>
          </div>
        </div>
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-5">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
            <div>
              ...
              <select v-model="attrs.form_type" class="w-full border border-gray-300 rounded-md px-2 py-1.5 text-sm">
                <option v-for="(l, k) in labels.formType" :key="k" :value="k">{{ l }}</option>
              </select>
            </div>
            <div>
              <label class="block text-xs text-gray-500 mb-1">契約状況</label>
              <select v-model="attrs.contract_status" class="w-full border border-gray-300 rounded-md px-2 py-1.5 text-sm">
            ...
            <div class="sm:col-span-3">
              ...
              <textarea v-model="attrs.remarks" rows="2" class="w-full border border-gray-300 rounded-md px-2 py-1.5 text-sm" />
            </div>
          </div>
          <div class="mt-3 text-right">
            <button @click="saveAttrs" class="px-4 py-1.5 text-xs bg-primary-500 text-white rounded-md hover:bg-primary-600">保存</button>
          </div>
        </div>
          ...
          <table class="w-full text-sm">
            ...
            <tbody class="divide-y">
              <tr v-for="d in management.details" :key="d.id" class="hover:bg-gray-50">
                <td class="px-4 py-3">
                  {{ d.facility_name }}
                  <span v-if="d.is_managing_facility" class="ml-1 text-xs bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded-md">管理</span>
                  <span v-if="isExternal(d)" class="ml-1 text-xs bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-md">外</span>
                </td>

                <template v-if="editing[d.id]">
                  <td class="px-4 py-2 text-right">
                    <input type="number" min="0" v-model.number="editing[d.id].total_amount"
                      class="w-28 text-right border border-gray-300 rounded-md px-2 py-1 text-sm" />
                  </td>
                  <td class="px-4 py-2 text-right">
                  ...
                  <td class="px-4 py-3 text-right whitespace-nowrap">
                    <button @click="saveEdit(d)" class="text-xs px-2 py-1 bg-primary-500 text-white rounded-md hover:bg-primary-600 mr-1">保存</button>
                    <button @click="cancelEdit(d)" class="text-xs px-2 py-1 border border-gray-300 rounded-md hover:bg-gray-50">取消</button>
                  </td>
                </template>
                ...
                <template v-else>
                  ...
                  <td class="px-4 py-3 text-right">
                    <button v-if="isExternal(d)" @click="startEdit(d)"
                      class="text-xs px-2 py-1 border border-gray-300 rounded-md hover:bg-gray-50 text-gray-600">編集</button>
                  </td>
                </template>
              </tr>
            </tbody>
