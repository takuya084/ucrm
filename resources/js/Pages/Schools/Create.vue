...
const store = () => {
  Inertia.post(route('schools.store'), form)
}
  ...
  <BreezeAuthenticatedLayout>
    <template #header>
      <div class="flex items-center gap-4">
        ...
        <h2 class="font-semibold text-xl text-gray-800">学校登録</h2>
      </div>
    </template>
        ...
        <div class="bg-white shadow-sm sm:rounded-lg p-6">
          ...
          <form @submit.prevent="store" class="space-y-5">
            ...
            <div>
              ...
              <input v-model="form.name" type="text" :class="inputClass" placeholder="例：○○市立△△小学校" />
            </div>

            <!-- 学校名（かな） -->
            <div>
              <label :class="labelClass">学校名（かな）</label>
              <input v-model="form.name_kana" type="text" :class="inputClass" placeholder="例：まるまるしりつさんかくしょうがっこう" />
            </div>

            <!-- 住所 -->
            <div>
              <label :class="labelClass">住所</label>
              <input v-model="form.address" type="text" :class="inputClass" placeholder="例：東京都渋谷区1-1-1" />
            </div>
            ...
            <div>
              ...
              <input v-model="form.memo" type="text" :class="inputClass" placeholder="備考など" />
            </div>
