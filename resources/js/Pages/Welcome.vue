...
// コンパクト表示のその他機能
const subFeatures = [
  { title: '空き枠調整', desc: '曜日ごとの利用枠の空きを視覚的に確認。欠席時の振替・補充候補がすぐ見つかります。' },
  { title: 'シフト管理', desc: '月間シフトの作成と人員配置基準のチェック。配置漏れをその場で確認できます。' },
  { title: '運営記録', desc: 'BCP・虐待防止委員会・身体拘束・自己評価の記録を管理。実地指導への備えに。' },
  { title: '問い合わせ管理', desc: '見学・体験のお問い合わせを一元管理。対応状況をステータスで追跡します。' },
]
...
<template>
  ...
  <div class="min-h-screen bg-white text-gray-800 font-sans antialiased">
    ...
    <header class="sticky top-0 z-50 bg-white border-b border-gray-200">
      <div class="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        <div class="flex items-center gap-2">
          ...
          <span class="text-[10px] bg-primary-600 text-white px-2 py-0.5 rounded-md font-bold tracking-wide">BETA</span>
        </div>
    ...
    <!-- ヒーロー -->
    <section class="bg-primary-50 border-b border-gray-200">
      <div class="max-w-5xl mx-auto px-6 pt-24 pb-20 text-center">
        <p class="inline-block text-xs font-bold text-primary-700 bg-white border border-primary-200 px-4 py-1.5 rounded-md mb-8 tracking-wide">
          放課後等デイサービス専用
        </p>
        ...
        <div class="flex flex-col sm:flex-row gap-4 justify-center items-center">
          <a href="#pricing"
            class="px-8 py-3.5 bg-primary-600 text-white text-sm font-bold rounded-md hover:bg-primary-700 transition">
            料金プランを見る
          </a>
          <a href="#features"
            class="px-8 py-3.5 text-sm font-bold text-primary-700 bg-white rounded-md border border-primary-300 hover:bg-primary-50 transition">
            機能を詳しく見る
          </a>
        </div>
      </div>
      ...
      <div class="max-w-5xl mx-auto">
        ...
        <div class="grid sm:grid-cols-3 gap-5">
          <div v-for="(item, i) in [
            ...
            class="relative bg-white rounded-lg p-6 border border-gray-200">
            <div class="w-10 h-10 bg-amber-50 rounded-md flex items-center justify-center mb-4">
      ...
      <div class="max-w-6xl mx-auto">
        ...
        <div class="space-y-12">
          <div v-for="(f, i) in features" :key="f.title"
            :class="[
              'flex flex-col gap-6 items-center bg-white rounded-lg border border-gray-200 overflow-hidden',
            ...
            <div class="sm:w-1/2 w-full bg-gray-100 aspect-video flex items-center justify-center shrink-0">
              ...
              <div v-else class="w-full h-full bg-primary-50 flex items-center justify-center select-none">
                <svg class="w-16 h-16 text-primary-200" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
            ...
            <div class="sm:w-1/2 w-full p-6 sm:p-10">
              <div class="inline-block text-[10px] font-bold text-primary-700 bg-primary-50 px-3 py-1 rounded-md mb-4 tracking-wide">
        ...
        <div class="mt-12 grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          <div v-for="s in subFeatures" :key="s.title"
            class="bg-white rounded-lg p-6 border border-gray-200 border-t-2 border-t-primary-500">
    ...
    <section class="py-20 px-6">
      <div class="max-w-4xl mx-auto">
        <div class="relative bg-primary-700 rounded-lg p-10 sm:p-16 text-white overflow-hidden">
          <div class="relative">
            <h2 class="text-2xl sm:text-3xl font-extrabold mb-4 leading-tight">
              記録の下書きを<br>AIが自動生成
            </h2>
            <p class="text-primary-100 leading-relaxed text-sm sm:text-base max-w-xl">
            ...
            <div class="mt-8 flex flex-wrap gap-3">
              <span v-for="tag in ['個別支援計画', 'モニタリング記録', '連絡帳の文章生成', '支援方針の提案']" :key="tag"
                class="text-xs bg-white/10 border border-white/20 text-white px-4 py-2 rounded-md">
                {{ tag }}
              </span>
            </div>
          </div>
    ...
    <section id="pricing" class="py-20 px-6 bg-gray-50">
      <div class="max-w-xl mx-auto text-center">
        ...
        <div class="bg-white rounded-lg border border-gray-200 p-10">
          <p class="text-xs font-bold text-primary-700 bg-primary-50 px-3 py-1 rounded-md inline-block mb-6 tracking-wide">すべて込み</p>
          ...
          <ul class="space-y-3 text-left max-w-xs mx-auto mb-10">
            <li v-for="item in [
              ...
              <svg class="w-4 h-4 text-primary-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
          ...
          <button @click="startCheckout" :disabled="isProcessing"
            class="block w-full py-3.5 bg-primary-600 text-white text-sm font-bold rounded-md hover:bg-primary-700 transition disabled:opacity-50">
    ...
    <section class="py-20 px-6">
      <div class="max-w-2xl mx-auto text-center">
        ...
        <button @click="startCheckout" :disabled="isProcessing"
          class="inline-block px-10 py-4 bg-primary-600 text-white text-sm font-bold rounded-md hover:bg-primary-700 transition disabled:opacity-50">
          {{ isProcessing ? '処理中...' : '申し込みはこちら' }}
        </button>
      </div>
    </section>
    ...
    <!-- フッター -->
    <footer class="py-10 px-6 border-t border-gray-200">
      <div class="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-gray-400">
        <span class="font-extrabold text-sm tracking-tight"><span class="text-primary-600">ハグ</span><span class="text-gray-900">くむ</span></span>
