    ...
    <template #header>
      <div class="flex items-center gap-4 flex-wrap">
        ...
        <span v-if="child.allergy_note"
          class="px-2 py-1 rounded text-xs font-medium text-red-700 bg-red-50 border border-red-200">
          ⚠ アレルギー
        </span>
        <div v-if="['admin','leader'].includes($page.props.auth.staff_role)" class="ml-auto flex gap-2">
          <Link :href="route('children.edit', child.id)" class="px-4 py-1.5 text-sm bg-indigo-500 text-white rounded hover:bg-indigo-600">
            編集
          </Link>
          <button @click="destroy" class="px-4 py-1.5 text-sm border border-red-300 text-red-500 rounded hover:bg-red-50">
            削除
          </button>
        </div>
      </div>
        ...
        <template v-if="activeTab === 'plan'">
          <div class="bg-white shadow-sm rounded-lg p-6">
            ...
            <div v-if="child.assessments?.length" class="space-y-2">
              ...
              >
                <div>
                  <Link :href="route('children.assessments.show', [child.id, a.id])" class="font-medium text-gray-800 hover:text-indigo-600">
        ...
        <template v-if="activeTab === 'usage'">
          <div class="bg-white shadow-sm rounded-lg p-6">
            <div class="flex justify-between items-center border-b pb-2 mb-4">
              ...
              <Link :href="route('usage-records.index')" class="text-xs text-indigo-600 hover:underline">出席管理へ →</Link>
            </div>
            <div v-if="child.usage_records?.length" class="space-y-1">
              ...
              >
                ...
                <Link
                  ...
                  class="ml-auto shrink-0 text-xs px-2 py-0.5 border border-indigo-200 text-indigo-600 rounded hover:bg-indigo-50"
                >支援記録</Link>
              </div>
            </div>
